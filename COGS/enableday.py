...
async def requirements(self, ctx, *args):
	...
	if not args[1].lower().title() in db[str(ctx.guild.id)][args[0]]["disdays"]:
		embedVar = tools.embed("This day is not disabled for this alarm", "Please type ``a!help`` to view the command usage if you need help.")
...
class events(commands.Cog):
	...
	@commands.command(aliases=["enblday", "ed", "enabledate", "enable"])
	async def enableday(self, ctx, *args):
		...
		embedVar = tools.embed("Day successfully resumed!", "This alarm will now send a notification on ``{}``.".format(args[1].lower().title()))
		await ctx.send(embed=embedVar)
